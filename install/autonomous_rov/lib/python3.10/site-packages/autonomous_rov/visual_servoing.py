...
class VisualServoing(Node):
    ...
    def compute_visual_servo(self):
        ...
        try:
            ...
            cam_speed = -self.P*1000000000000000000 * L_pinv.dot(error)
        except:
        ...
        self.setOverrideRCIN(1500, 1500, 1500,
                              yaw_left_right, self.thruttle, 1500)

        self.get_logger().info(f' Published new command to RCIN {0}, {0}, {0},'
                               f'{yaw_left_right}, {self.thruttle}, {0}')
    ...
    def setOverrideRCIN(self, channel_pitch, channel_roll, channel_throttle, channel_yaw, channel_forward, channel_lateral):
        ...
        # self.get_logger().info(f"Direction of movement: "
        #                        f"Roll: {self.direction(channel_roll, 'roll')}, "
        #                        f"Pitch: {self.direction(channel_pitch, 'pitch')}, "
        #                        f"Throttle: {self.direction(channel_throttle, 'throttle')}, "
        #                        f"Yaw: {self.direction(channel_yaw, 'yaw')}, "
        #                        f"Forward: {self.direction(channel_forward, 'forward')}, "
        #                        f"Lateral: {self.direction(channel_lateral, 'lateral')}")


    def direction(self, channel_value, axis_name):
        if axis_name == 'roll':
            if channel_value > 1500:
                return f"moving towards right ({axis_name})"
            elif channel_value < 1500:
                return f"moving towards left ({axis_name})"
            else:
                return f"no movement along {axis_name}"

        elif axis_name == 'pitch':
            if channel_value > 1500:
                return f"moving towards downwards ({axis_name})"
            elif channel_value < 1500:
                return f"moving towards upwards ({axis_name})"
            else:
                return f"no movement along {axis_name}"

        elif axis_name == 'throttle':
            if channel_value > 1500:
                return f"moving towards increase in throttle ({axis_name})"
            elif channel_value < 1500:
                return f"moving towards decrease in throttle ({axis_name})"
            else:
                return f"no movement along {axis_name}"

        elif axis_name == 'yaw':
            if channel_value > 1500:
                return f"moving towards right yaw ({axis_name})"
            elif channel_value < 1500:
                return f"moving towards left yaw ({axis_name})"
            else:
                return f"no movement along {axis_name}"

        elif axis_name == 'forward':
            if channel_value > 1500:
                return f"moving forward ({axis_name})"
            elif channel_value < 1500:
                return f"moving backward ({axis_name})"
            else:
                return f"no movement along {axis_name}"

        elif axis_name == 'lateral':
            if channel_value > 1500:
                return f"moving towards right ({axis_name})"
            elif channel_value < 1500:
                return f"moving towards left ({axis_name})"
